...
export default function Portfolio() {
  return (
    <main className="min-h-screen bg-slate-950 text-white overflow-hidden">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/70 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <a href="#home" className="text-lg font-bold tracking-tight">
            Ayan<span className="text-cyan-400">.</span>
          </a>
      ...
      <section id="home" className="relative z-10 mx-auto grid min-h-[88vh] max-w-6xl items-center gap-10 px-6 py-20 md:grid-cols-[1.2fr_0.8fr]">
        <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.7 }}>
          ...
          <h1 className="max-w-3xl text-5xl font-black leading-tight tracking-tight md:text-7xl">
            Hi, I&apos;m <span className="bg-gradient-to-r from-cyan-300 via-violet-300 to-fuchsia-300 bg-clip-text text-transparent">Ayan Akhundova</span>
          </h1>
          ...
          <div className="mt-8 flex gap-4">
            <a href="https://github.com/yourusername" aria-label="GitHub" className="rounded-xl border border-white/10 bg-white/5 p-3 text-slate-300 transition hover:bg-white/10 hover:text-white">
              <Github size={22} />
            </a>
            <a href="https://linkedin.com/in/yourusername" aria-label="LinkedIn" className="rounded-xl border border-white/10 bg-white/5 p-3 text-slate-300 transition hover:bg-white/10 hover:text-white">
              <Linkedin size={22} />
            </a>
            <a href="mailto:your.email@example.com" aria-label="Email" className="rounded-xl border border-white/10 bg-white/5 p-3 text-slate-300 transition hover:bg-white/10 hover:text-white">
              <Mail size={22} />
            </a>
          </div>
        </motion.div>
  ...
  learning: true,
      ...
      <section id="about" className="relative z-10 mx-auto max-w-6xl px-6 py-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }} className="grid gap-6 md:grid-cols-[0.8fr_1.2fr]">
          <div>
            ...
            <h2 className="text-4xl font-bold tracking-tight">Creating interfaces that are simple, clear, and useful.</h2>
          </div>
          <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-8 text-slate-300 shadow-xl shadow-black/20">
            <p className="leading-8">
              I am a junior frontend developer focused on building modern web applications. I work with HTML, CSS, JavaScript, React, Git, and GitHub. I am interested in responsive design, UI/UX, and writing clean code that is easy to understand and maintain.
            </p>
            <p className="mt-4 leading-8">
              I have experience working on personal and team projects, including admin panels, landing pages, and React applications. My goal is to grow as a frontend developer and contribute to real products with useful and polished interfaces.
            </p>
          </div>
        </motion.div>
      </section>
      ...
      <section id="skills" className="relative z-10 mx-auto max-w-6xl px-6 py-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }}>
          ...
          <h2 className="text-4xl font-bold tracking-tight">Technologies I use</h2>
          <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
            {skills.map((skill) => (
              <div key={skill} className="rounded-2xl border border-white/10 bg-white/[0.06] px-5 py-4 text-center font-medium text-slate-200 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-cyan-300/10">
                {skill}
              </div>
            ))}
          </div>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }}>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {projects.map((project) => (
              <article key={project.title} className="group flex flex-col rounded-3xl border border-white/10 bg-white/[0.06] p-6 shadow-xl shadow-black/20 transition hover:-translate-y-2 hover:border-cyan-300/40">
                ...
                <div className="mt-6 flex gap-3">
                  <a href={project.github} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-200 transition hover:bg-white/10">
                    <Github size={16} /> Code
                  </a>
                  <a href={project.demo} className="inline-flex items-center gap-2 rounded-xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300">
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-6 py-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }}>
          ...
          <h2 className="text-4xl font-bold tracking-tight">Education & certificates</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {certificates.map((certificate) => (
              <div key={certificate} className="rounded-2xl border border-white/10 bg-white/[0.06] p-5 text-slate-200">
                {certificate}
              </div>
            ))}
          </div>
      ...
      <section id="contact" className="relative z-10 mx-auto max-w-6xl px-6 py-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.6 }} className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/[0.09] to-white/[0.04] p-8 text-center shadow-2xl shadow-black/20 md:p-12">
          ...
          <p className="mx-auto mt-4 max-w-2xl leading-8 text-slate-300">
            I am open to junior frontend developer opportunities, internships, freelance tasks, and team projects.
          </p>
          <a href="mailto:your.email@example.com" className="mt-8 inline-flex items-center justify-center gap-2 rounded-2xl bg-cyan-400 px-6 py-3 font-semibold text-slate-950 shadow-lg shadow-cyan-400/20 transition hover:-translate-y-0.5 hover:bg-cyan-300">
